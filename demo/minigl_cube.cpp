...
int main( void )
{
    ...
    my_window.render([&]{
        ...
        if (current_time - last_time > .01){
            ...
            pipelines[pipelines.size()-1]["MVP"] = Projection* View * translate(Model, vec3(0,0,0.001*counter));
            pipelines[pipelines.size()-1]["vertex_position"] = g_vertex_buffer_data;
            pipelines[pipelines.size()-1]["vertex_color"] = g_color_buffer_data;
            counter++;
        ...
        {
            t.render(my_window.width(), my_window.height());
        }
    });
    return 0;
}